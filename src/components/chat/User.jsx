...
export default function User({ listUser }) {
  ...
  return (
    <section className="flex flex-col gap-4 pr-4">
      ...
      <div className={`flex flex-col gap-4 h-[810px] overflow-auto ${listUser?.user?.getList?.data > 10 ? 'pr-4' : 'pr-0'}`}>
        {filteredUsers.length === 0 && <span className="xl:w-[381px]">data tidak ada</span>}

        {filteredUsers?.map((user) => (
          <div key={user?.id} className="bg-[#F8F6F4] relative flex flex-row items-start p-4 gap-4 rounded-lg xl:max-w-lg duration-200 hover:bg-[#E9E9E9] cursor-pointer">
            <img
            ...
            <div className="flex flex-col gap-1.5">
              <h1 className="m-0 font-semibold text-lg capitalize text-[#393E46]">{user?.name}</h1>
              <p className="m-0 text-[#A0A0A0] text-sm">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quis, ut!</p>
            </div>
